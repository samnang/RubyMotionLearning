class ColorController < UIViewController
  ...
  def viewDidLoad
    ...
    @info_container = UIView.alloc.initWithFrame([[0, 0], [view.frame.size.width, 60]])
    ...
    @color_view.backgroundColor = String.new(color.hex).to_color
    ...
    @add.sizeToFit
    @add.frame = [
      [view.frame.size.width - @add.frame.size.width - padding, @color_view.frame.origin.y],
    ...
    @text_field.frame = [
      text_field_origin,
      [view.frame.size.width - text_field_origin[0] - add_button_offset, @color_view.frame.size.height]
    ]

    table_height = view.bounds.size.height - @info_container.frame.size.height
    table_frame = [
